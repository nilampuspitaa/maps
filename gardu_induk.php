    ...
    <div id=mymap></div>
      ...
      <script>
          ...
          function initMap(){
            ...
            xmlhttp.onreadystatechange = function(){
              if(this.readyState == 4 && this.status == 200){
                ...
                for(i=0; i<data.length; i++){
                  var location = new google.maps.LatLng(parseFloat(data[i].GI_KOORX), parseFloat(data[i].GI_KOORY));
                  marker = new google.maps.Marker({
                    position: location,
                    map: map,
                    label: data[i].GI_NAMA
                  }); 
            ...
            xmlhttp.open("GET", "http://localhost/maps/api/data_gardu_induk.php", true);
            xmlhttp.send();
          }
