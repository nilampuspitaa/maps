...
<html lang=en>
 ...
 #mymap {
  ...
  <body>
    <h3> Google Maps COD Gardu Induk</h3>
    <div id=mymap></div>
      ...
      <script>
          ...
          function initMap(){
            ...
            xmlhttp.onreadystatechange = function(){
              if(this.readyState == 4 && this.status == 200){
                ...
                for(i=0; i<data.length; i++){
                  var location = new google.maps.LatLng(parseFloat(data[i].CODGI_KOORX), parseFloat(data[i].CODGI_KOORY));
                  marker = new google.maps.Marker({
                    position: location,
                    map: map,
                    label: data[i].CODGI_NAMA
                  }); 
            ...
            xmlhttp.open("GET", "http://localhost/maps/api/data_codgi.php", true);
            xmlhttp.send();
          }
