    ...
    <div id=mymap></div>
      ...
      <script>
          ...
          function initMap(){
            ...
            xmlhttp.onreadystatechange = function(){
              if(this.readyState == 4 && this.status == 200){
                ...
                for(i=0; i<data.length; i++){
                  var location = new google.maps.LatLng(parseFloat(data[i].TR_KOORX), parseFloat(data[i].TR_KOORY));
                  marker = new google.maps.Marker({
                    position: location,
                    map: map,
                    label: data[i].TR_NAMA_BAY
                  }); 
            ...
            xmlhttp.open("GET", "http://localhost/maps/api/data_transmisi.php", true);
            xmlhttp.send();
          }
